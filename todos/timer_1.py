...
class Child(QWidget):
    ...
    def button_delete(self):
        ...
        # reply = QMessageBox.question(self, '确认操作', '确定要删除这项任务吗？', QMessageBox.Yes | QMessageBox.Cancel,
        #                              QMessageBox.Cancel)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # # 按钮-添加倒计时任务
        # self.bt_add = QPushButton("添加任务", self)
        # self.bt_add.clicked.connect(self.add_buttonClicked)
        # self.bt_add.adjustSize()
        ...
        # # 按钮-重置选中任务
        # self.bt_reset_all = QPushButton("重置选中任务", self)
        # self.bt_reset_all.clicked.connect(self.reset_all_buttonClicked)
        # self.bt_reset_all.adjustSize()

        # 连接数据库、创建表tasks
        create_table()

        # 表格
        # print('表格接收到的data', self.data)
        # 设置数据层次结构，行数，4列

        # self.model = QStandardItemModel(len(self.data), 4)
        # self.model.setHorizontalHeaderLabels(['任务名称', '总时长', '剩余时长', '控制'])
        ...
        # # self.tableView.customContextMenuRequested[QPoint].connect(self.right_click_table)

        # 厢式布局/框布局
        # 水平盒子1，装按钮
        h_box1 = QHBoxLayout()
        # h_box1.addWidget(self.bt_add)
        # h_box1.addWidget(self.bt_reset_all)
        h_box1.addStretch(1)
        # 垂直盒子
        v_box = QVBoxLayout()
        v_box.addLayout(h_box1)
    ...
    def table_double_clicked(self, index):
        ...
        if self.data[row][8] == 0:
            self.data[row][8] = 1
            # self.model.setItem(row, 3, QStandardItem('单击暂停'))
            # 更新开始时间和状态
            ...
            for row in range(len(self.data)):
                ...
                for column in range(4):
                    ...
                    if column == 3:
                        ...
                        # 这个函数可以用来定义所有控件的
                        # bt_box.setStyleSheet(
                        #     'QWidget{height:40;margin:1px;font-family:Times New Roman;font-size:14;border-radius:8px}')
                        # print(bt_box.height())
                        # print('\t',self.bt_del.height())
    ...
    def get_data(self):
        ...
        # 查所有的项
        sql = 'select id,name,totaltime,resttime,laststart,lastpause,lastcomplete,completecount,status from tasks order by id asc '
        cursor.execute(sql)
